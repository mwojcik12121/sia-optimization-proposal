...
while (( $# > 0 )); do
  case "$1" in
    ...
    --renterd) RENTERD_REPO="$(build_option_value "$1" "$#" "${2:-}")"; shift 2 ;;
    --renterd=*) RENTERD_REPO="${1#*=}"; shift ;;
...
repo_names=("${CORE_REPO}" "${COREUTILS_REPO}" "${HOSTD_REPO}" "${RENTERD_REPO}" "${WALLETD_REPO}")
[[ "$(printf '%s\n' "${repo_names[@]}" | sort -u | wc -l)" -eq 5 ]] \
...
build_require_repository --renterd "${RENTERD_REPO}" go.sia.tech/renterd/v2 renterd
build_require_repository --walletd "${WALLETD_REPO}" go.sia.tech/walletd/v2 walletd

for command in docker tar awk sort wc mktemp date; do
...
export CORE_REPO COREUTILS_REPO HOSTD_REPO RENTERD_REPO WALLETD_REPO BUILD_JOBS
...
printf '[builder] Compiling hostd, renterd, and walletd with --build-jobs=%s and Docker networking disabled.\n' "${BUILD_JOBS}" >&2
...
docker build \
  ...
  --build-arg "RENTERD_REPO=${RENTERD_REPO}" \
