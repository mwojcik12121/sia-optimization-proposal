...
func (tx *MainDatabaseTx) WalletAddBroadcastedSet(ctx context.Context, txnSet wallet.BroadcastedSet) error {
	_, err := tx.Tx.Exec(ctx, "INSERT INTO wallet_broadcasted_txnsets (created_at, block_id, height, txn_set_id, raw_transactions) VALUES (?, ?, ?, ?, ?) ON CONFLICT (txn_set_id) DO NOTHING",
