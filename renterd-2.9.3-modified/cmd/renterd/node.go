...
func newAutopilot(masterKey [32]byte, cfg config.Autopilot, bus *bus.Client, l *zap.Logger) (Autopilot, error) {
	...
	m, err := migrator.New(ctx, masterKey, a, bus, bus, cfg.MigratorHealthCutoff, cfg.SlabRisk, cfg.MigratorNumThreads, cfg.MigratorDownloadMaxOverdrive, cfg.MigratorUploadMaxOverdrive, cfg.MigratorDownloadOverdriveTimeout, cfg.MigratorUploadOverdriveTimeout, cfg.MigratorUploadSectorTimeout, cfg.MigratorAccountsRefillInterval, l)
	if err != nil {
		cancel(nil)
		return nil, err
	}
...
func newBus(cfg config.Config, pk types.PrivateKey, network *consensus.Network, genesis types.Block, instantSync bool, logger *zap.Logger) (*bus.Bus, func(ctx context.Context) error, error) {
	...
	if instantSync && !consensusExists {
		...
		if checkpoint.Height > blocksPerYear {
			...
			// safe lower bound.
			heightWithAnnouncements = max(checkpoint.Height-blocksPerYear, network.HardforkV2.RequireHeight)
		}
		...
		if instantSync {
			logger.Warn("skipping instant sync: consensus database already exists")
		}
		bdb, err = coreutils.OpenBoltChainDB(chainPath)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to open chain database: %w", err)
	...
	// create wallet
	w, err := cwallet.NewSingleAddressWallet(pk, cm, sqlStore, s, cwallet.WithReservationDuration(cfg.Bus.UsedUTXOExpiry), cwallet.WithDefragThreshold(250))
	if err != nil {
		return nil, nil, err
	}
